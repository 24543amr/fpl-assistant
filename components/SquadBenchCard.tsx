...
export default function SquadBenchCard({
  ...
  // Points color styling: green >=7, yellow 2-6, red 0-1, dash if 0
  const pts = points ?? (player?.total_points || 0);
  const isHigh = pts >= 7;
  const isMed = pts >= 2 && pts < 7;

  const ptsBg = isHigh ? '#34ff8c' : isMed ? '#f6c343' : pts > 0 ? 'rgba(147, 0, 10, 0.7)' : '#333535';
  const ptsColor = isHigh ? '#003919' : isMed ? '#281a00' : pts > 0 ? '#ffdad6' : '#9b8d97';
